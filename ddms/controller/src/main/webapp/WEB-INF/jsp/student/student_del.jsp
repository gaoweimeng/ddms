...
<script>
    layui.use('table', function () {
        ...
        table.render({
            ...
            , url: '${ctx}/student/selectAllStudent?delFlag=1'
            , title: '数据恢复表'
            , id: 'delStudentList'
            , toolbar: '#toolbarDemo'
            , cols: [[
                {type: 'checkbox', fixed: 'left'}  //选择框
                , {field: 'id', title: '学号', sort: true, width: 140}
                , {field: 'stuName', title: '学生姓名', mimWidth: 90}
                , {field: 'stuSex', title: '性别', width: 60}
                , {field: 'stuPhone', title: '手机号'}
                , {field: 'majorName', title: '所属专业', minWidth: 150}
        ...
        table.on('tool(test)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'see') {
                ...
                layer.confirm('臣妾错了，能不能别让我走 ＞﹏＜？', {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: '${ctx}/student/deleteStudentSecond',
                        dataType: 'json',
                        data : {
                            id : data.id
                        },
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                case 'dataRecovery':
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '${ctx}/student/restoreStudent',
                        traditional: true,
                        data: {
                            id : ids
                        },
